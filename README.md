...
**Current Version:** 2.2.0 (in development — `v2.2.0-rc5` cut for pre-production testing; completes Story E-1 with the startup/reconnect **backfill** + the 4-iteration review hardening of the event-stream value path, [#130](https://github.com/guycorbaz/opcgw/issues/130)). v2.1.0 shipped web-first configuration & auto-discovery (Epic C first-run setup wizard, ChirpStack inventory pickers, duplicate-prevention, drift view, TOML→SQLite config migration; Epic D SQLite singleton configuration + web editor). v2.2.0 brings **Epic E — the model-agnostic, class-aware device-abstraction layer**: Story E-0 wires the downlink command path end-to-end (an OPC UA command write is delivered to the device via ChirpStack); Story E-1 adds **uplink-event ingestion with last-known values and no gateway-side aggregation** ([#130](https://github.com/guycorbaz/opcgw/issues/130)) — opcgw reports each device value as its raw last reading with the device's source timestamp, and the metrics poll no longer aggregates streamed devices (set `chirpstack.stream_all_devices = true` to de-aggregate the whole fleet), plus an optional per-device `stale_threshold_seconds` ([#132](https://github.com/guycorbaz/opcgw/issues/132)). The `v2.2.0-rc5` tag publishes a `2.2.0-rc5` Docker image (it does **not** move the `latest`/`2.1` tags). **The real-world Tonhe E20 valve gate PASSED on rc4 (2026-06-11)**: a full OPEN **and** CLOSE cycle was driven from Fuxa/OPC UA through opcgw (`command_class = "valve"` set in the web editor) and the valve physically actuated both ways — Story E-0 is `done`. De-aggregated-read validation (E-1 / AC#11) already passed on rc2; rc5 completes Story E-1 (startup/reconnect **backfill** + a freshness guard across the whole stream value path, hardened by a 4-iteration code review) — **the rc5 gate is the cold-start check** (values on OPC UA immediately after a restart, via the backfill), the last step before E-1 flips `done` and v2.2.0 can go stable ([#130](https://github.com/guycorbaz/opcgw/issues/130)). The test also surfaced that command dispatch waits for the next metrics-poll cycle — tracked as CR [#136](https://github.com/guycorbaz/opcgw/issues/136) (decouple dispatch from the poll, backlog). See [`CHANGELOG.md`](./CHANGELOG.md) for full release notes.

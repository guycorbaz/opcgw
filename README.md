...
**Current Version:** 2.2.0 (in development — `v2.2.0-rc2` cut for pre-production testing). v2.1.0 shipped web-first configuration & auto-discovery (Epic C first-run setup wizard, ChirpStack inventory pickers, duplicate-prevention, drift view, TOML→SQLite config migration; Epic D SQLite singleton configuration + web editor). v2.2.0 brings **Epic E — the model-agnostic, class-aware device-abstraction layer**: Story E-0 wires the downlink command path end-to-end (an OPC UA command write is delivered to the device via ChirpStack); Story E-1 adds **uplink-event ingestion with last-known values and no gateway-side aggregation** ([#130](https://github.com/guycorbaz/opcgw/issues/130)) — opcgw reports each device value as its raw last reading with the device's source timestamp, and the metrics poll no longer aggregates streamed devices (set `chirpstack.stream_all_devices = true` to de-aggregate the whole fleet). The `v2.2.0-rc2` tag publishes a `2.2.0-rc2` Docker image (it does **not** move the `latest`/`2.1` tags) for a real-world Tonhe E20 valve OPEN/CLOSE test (E-0 / AC#10) **and** de-aggregated-read validation (E-1 / AC#11) before E-1 flips to `done`. See [`CHANGELOG.md`](./CHANGELOG.md) for full release notes.

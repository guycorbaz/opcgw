...
**Important — the wizard collects ONLY the OPC UA `user_password`.** Every other configuration field (ChirpStack credentials, OPC UA settings + PKI dir + application name, web settings, etc.) must still be present in `config/config.toml` (or provided via `OPCGW_*` env-vars) before the gateway starts. A truly-empty `config.toml` will fail validation BEFORE the wizard becomes reachable. The minimum bootstrap `config.toml` shape:

```toml
[chirpstack]
server_address = "http://127.0.0.1:8080"
api_token = "<your-token>"            # or set OPCGW_CHIRPSTACK__API_TOKEN
...
**Current Version:** 2.2.0 (**released 2026-06-13** — tagged `v2.2.0`, Docker `2.2.0` / `2.2` / `latest` on Docker Hub + GHCR. rc5 completed Story E-1 ([#130](https://github.com/guycorbaz/opcgw/issues/130)) and passed the cold-start gate in production; the rc6 CommandStatusPoller NULL-row fix ([#134](https://github.com/guycorbaz/opcgw/issues/134)) then soaked clean in pre-production — zero ERROR lines on 06-13, poller alive — clearing the last gate to stable. Epic E remaining: E-3 delivery confirmation + retrospective.) v2.1.0 shipped web-first configuration & auto-discovery (Epic C first-run setup wizard, ChirpStack inventory pickers, duplicate-prevention, drift view, TOML→SQLite config migration; Epic D SQLite singleton configuration + web editor). v2.2.0 brings **Epic E — the model-agnostic, class-aware device-abstraction layer**: Story E-0 wires the downlink command path end-to-end (an OPC UA command write is delivered to the device via ChirpStack); Story E-1 adds **uplink-event ingestion with last-known values and no gateway-side aggregation** ([#130](https://github.com/guycorbaz/opcgw/issues/130)) — opcgw reports each device value as its raw last reading with the device's source timestamp, and the metrics poll no longer aggregates streamed devices (set `chirpstack.stream_all_devices = true` to de-aggregate the whole fleet), plus an optional per-device `stale_threshold_seconds` ([#132](https://github.com/guycorbaz/opcgw/issues/132)). The `v2.2.0` stable tag moves `latest` / `2.2` (rc tags did not). **The real-world Tonhe E20 valve gate PASSED on rc4 (2026-06-11)**: a full OPEN **and** CLOSE cycle was driven from Fuxa/OPC UA through opcgw (`command_class = "valve"` set in the web editor) and the valve physically actuated both ways — Story E-0 is `done`. De-aggregated-read validation (E-1 / AC#11) passed on rc2; rc5 completes Story E-1 (startup/reconnect **backfill** + a freshness guard across the whole stream value path, hardened by a 4-iteration code review) and **the rc5 cold-start gate PASSED in production (2026-06-12)**: immediately after boot the gateway restored all 40 metrics from SQLite before the poller started, the backfill's freshness guard correctly declined to overwrite them, and live uplinks kept flowing with zero stream drops — **Story E-1 is `done`** ([#130](https://github.com/guycorbaz/opcgw/issues/130)). rc6 fixes the last operational blemish before stable: downlink commands queued from OPC UA left NULL columns that crashed the delivery-confirmation poll every 5 s ([#134](https://github.com/guycorbaz/opcgw/issues/134)) — confirmed gone in pre-production (zero ERROR lines on 06-13, poller alive), and **v2.2.0 shipped stable on 2026-06-13**. The valve test also surfaced that command dispatch waits for the next metrics-poll cycle — tracked as CR [#136](https://github.com/guycorbaz/opcgw/issues/136) (decouple dispatch from the poll, backlog). See [`CHANGELOG.md`](./CHANGELOG.md) for full release notes.
...
| **Epic F — Onboarding & Web UX for Public Release** | 📋 backlog (5 stories F-0..F-4; opened 2026-06-14) — make opcgw effortless to configure and pleasant to use **before announcing it to the ChirpStack team**. Config-in-database is already done (Epics C/D); this epic targets the **first-run experience** and the **apply model**. **F-0** staged config + explicit **"Apply changes"** soft restart (edits accumulate in SQLite without restarting; one operator-initiated graceful in-process supervisor restart applies the batch; the container is never restarted — absorbs CR [#138](https://github.com/guycorbaz/opcgw/issues/138)). **F-1** unified web shell (vanilla, **no build step / framework / `node_modules`** — a deliberate asset for an auditable industrial gateway). **F-2** zero-touch first-run wizard (`/setup` captures ChirpStack connection/token/tenant + OPC UA so an empty `config.toml`/`.env` boots browser-only; web-UI login user/password and log-file location stay in `.env` by design). **F-3** dashboard landing redesign. **F-4** config export/import (secrets excluded by default). Recommended order F-0 → F-1 → F-2 → F-3 → F-4. Tracked as GH [#140](https://github.com/guycorbaz/opcgw/issues/140). |

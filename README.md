...
**Current Version:** 2.0.0 — last updated 2026-04-30 (Story 8-1 done; 8-2 in review).
...
| **Epic 8 — Real-Time Subscriptions & Historical Data (Phase B)** | 🔄 in-progress (8-1 done; 8-2 in review) | **8-1 done** — `async-opcua` 0.17.1 subscription spike confirmed Plan A with zero `src/` changes; 9 integration tests pin the contract. **8-2 in review (2026-04-30)** — four mandatory `Limits` knobs (`max_subscriptions_per_session`, `max_monitored_items_per_sub`, `max_message_size`, `max_chunk_count`) plumbed through `OpcUaConfig` → `ServerBuilder`; 20 validation unit tests + 3 integration tests; cross-knob coherence check + diagnostic `event="opcua_limits_configured"` startup emit; NFR12 carry-forward intact (zero new audit events). See [`8-2-opc-ua-subscription-support.md`](./_bmad-output/implementation-artifacts/8-2-opc-ua-subscription-support.md) for full File List + Dev Notes; [`8-1-spike-report.md`](./_bmad-output/implementation-artifacts/8-1-spike-report.md) for the spike. Stories 8-3 / 8-4 still backlog. |

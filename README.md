...
**Current Version:** 2.2.0 (in development — `v2.2.0-rc6` cut for pre-production testing; rc5 completed Story E-1 ([#130](https://github.com/guycorbaz/opcgw/issues/130)) and passed the cold-start gate in production; rc6 adds the CommandStatusPoller NULL-row fix, [#134](https://github.com/guycorbaz/opcgw/issues/134)). v2.1.0 shipped web-first configuration & auto-discovery (Epic C first-run setup wizard, ChirpStack inventory pickers, duplicate-prevention, drift view, TOML→SQLite config migration; Epic D SQLite singleton configuration + web editor). v2.2.0 brings **Epic E — the model-agnostic, class-aware device-abstraction layer**: Story E-0 wires the downlink command path end-to-end (an OPC UA command write is delivered to the device via ChirpStack); Story E-1 adds **uplink-event ingestion with last-known values and no gateway-side aggregation** ([#130](https://github.com/guycorbaz/opcgw/issues/130)) — opcgw reports each device value as its raw last reading with the device's source timestamp, and the metrics poll no longer aggregates streamed devices (set `chirpstack.stream_all_devices = true` to de-aggregate the whole fleet), plus an optional per-device `stale_threshold_seconds` ([#132](https://github.com/guycorbaz/opcgw/issues/132)). The `v2.2.0-rc6` tag publishes a `2.2.0-rc6` Docker image (it does **not** move the `latest`/`2.1` tags). **The real-world Tonhe E20 valve gate PASSED on rc4 (2026-06-11)**: a full OPEN **and** CLOSE cycle was driven from Fuxa/OPC UA through opcgw (`command_class = "valve"` set in the web editor) and the valve physically actuated both ways — Story E-0 is `done`. De-aggregated-read validation (E-1 / AC#11) passed on rc2; rc5 completes Story E-1 (startup/reconnect **backfill** + a freshness guard across the whole stream value path, hardened by a 4-iteration code review) and **the rc5 cold-start gate PASSED in production (2026-06-12)**: immediately after boot the gateway restored all 40 metrics from SQLite before the poller started, the backfill's freshness guard correctly declined to overwrite them, and live uplinks kept flowing with zero stream drops — **Story E-1 is `done`** ([#130](https://github.com/guycorbaz/opcgw/issues/130)). rc6 fixes the last operational blemish before stable: downlink commands queued from OPC UA left NULL columns that crashed the delivery-confirmation poll every 5 s ([#134](https://github.com/guycorbaz/opcgw/issues/134)) — the rc6 deploy check is that this ERROR is gone from the logs, after which **v2.2.0 goes stable**. The valve test also surfaced that command dispatch waits for the next metrics-poll cycle — tracked as CR [#136](https://github.com/guycorbaz/opcgw/issues/136) (decouple dispatch from the poll, backlog). See [`CHANGELOG.md`](./CHANGELOG.md) for full release notes.

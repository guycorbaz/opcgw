...
**Current Version:** 2.0.0 — last updated 2026-05-04 (Story 9-0 in review — async-opcua runtime address-space mutation spike. Three load-bearing questions resolved empirically: Q1 add path RESOLVED FAVOURABLY, Q2 remove path Behaviour B (frozen-last-good — 9-8 must arrange explicit cleanup), Q3 sibling isolation RESOLVED FAVOURABLY (write-lock-hold = 117 µs for 11 mutations). `OpcUa::run` split into `build` + `run_handles` to give 9-7 hot-reload a clean integration seam. Spike report at `_bmad-output/implementation-artifacts/9-0-spike-report.md`. Previous: Story 9-3 done after iter-1 + iter-2 code review; live metric values page at /metrics.html with per-row staleness badges; #107 duplicate-metric_name validation gap and #108 payload-less MetricType which BLOCKS production deployment until storage trait refactor lands.).

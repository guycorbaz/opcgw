...
- **Devices can no longer be saved with a placeholder DevEUI** (#125). The device CRUD API now rejects `device_id` capture-failure sentinels (`undefined`, `null`, `NaN`, `none`) that the web picker could leak when it failed to capture a DevEUI — such a device can never poll (ChirpStack rejects it with `"Odd number of digits"`). Real DevEUIs and existing free-form ids are unaffected.

...
> Docker `gcorbaz/opcgw:2.7.1-rc2` + GHCR mirror (multi-arch amd64+arm64); **not**
> `:latest` / `:2.7`. Builds on `v2.7.1-rc1` (the `max_keep_alive_count` knob) by
> making it editable from the web UI and reorganizing the config page.

...
- [ ] **Task 0 — Tracking issue acknowledgment (AC: #23)**
...
- [ ] **Task 1 — Dual-registry multi-arch CI workflow (AC: #1, #2, #3, #4, #5, #6)**
  - [ ] 1.1 Add `docker/setup-qemu-action@v3` step before the existing `docker/setup-buildx-action@v3` step in `.github/workflows/docker-build.yml`.
  - [ ] 1.2 Add a `docker/login-action@v3` step for Docker Hub using `secrets.DOCKERHUB_USERNAME` + `secrets.DOCKERHUB_TOKEN`; preserve the existing GHCR login step (uses `github.actor` + `secrets.GITHUB_TOKEN`).
  - [ ] 1.3 Extend the `docker/metadata-action@v5` `images:` list to include both `docker.io/gcorbaz/opcgw` and `ghcr.io/${{ github.repository }}` (one entry per line). Tag rules unchanged.
  - [ ] 1.4 Add `platforms: linux/amd64,linux/arm64` to the `docker/build-push-action@v5` step.
  - [ ] 1.5 **Decision point** — choose Docker Hub long-description sync approach:
  ...
  - [ ] 1.6 Render-check the YAML locally (`yq` or `python -c 'import yaml; yaml.safe_load(open(...))'`) before commit.

- [ ] **Task 2 — Dockerfile hardening (AC: #7, #8, #9)**
  - [ ] 2.1 Uncomment the `useradd` block at lines 34-40 (remove leading `#` from `RUN useradd ...` block) and the `USER opcgw` line; remove the now-redundant `#` from the `ARG UID=10001` line if commented.
  - [ ] 2.2 Pin runtime base: change `FROM ubuntu:latest` to `FROM ubuntu:24.04`.
  - [ ] 2.3 Verify the binary + static/ directory are world-readable post-COPY (typically yes by default); add `chmod` / `chown` only if Task 3 smoke-test reveals a permission failure.
  - [ ] 2.4 Confirm `docker-compose.yml` bind-mounts (`./log`, `./config`, `./pki`) remain compatible with UID 10001 — if not, document the operator-side `chown 10001:10001 ./log ./config ./pki` step in the manual's Installation chapter.

- [ ] **Task 3 — Local Docker smoke test (AC: #10)**
  - [ ] 3.1 Build: `docker build -t opcgw:smoke .` (capture build time + final image size in Dev Notes).
  - [ ] 3.2 Prepare a minimal `tests/smoke-config.toml` (or reuse `config/config.example.toml`) with a stub ChirpStack endpoint that won't actually be reached, but lets the gateway start cleanly.
  - [ ] 3.3 Run: `docker run --rm --name opcgw-smoke -p 4855:4855 -e OPCGW_CHIRPSTACK__API_TOKEN=smoke -e OPCGW_OPCUA__USER_PASSWORD=smoke -v "$(pwd)/tests:/usr/local/bin/config:ro" opcgw:smoke &`.
  - [ ] 3.4 Verify `docker exec opcgw-smoke id` reports `uid=10001(opcgw) gid=...` (NOT root). Capture output in Dev Notes.
  - [ ] 3.5 Verify port 4855 is listening: `nc -z localhost 4855` returns 0.
  - [ ] 3.6 Verify gateway logs at least one poll-attempt event within 30 s (grep the container logs for `chirpstack` events).
  - [ ] 3.7 Clean up: `docker stop opcgw-smoke`.

- [ ] **Task 4 — Docker Hub long-description page (AC: #6, #11)**
  - [ ] 4.1 Create `docs/dockerhub-description.md` (new file, Markdown).
  - [ ] 4.2 Content sections (target ≤500 lines, Docker Hub renders Markdown):
  ...
  - [ ] 4.3 Header comment (HTML comment) documenting the sync approach chosen in Task 1.5.

- [ ] **Task 5 — Repository README.md update (AC: #11, #12)**
  - [ ] 5.1 Add a new "Docker" section near the top of `README.md` (after the badges, before "Architecture" or wherever fits the existing flow). Include both `docker pull` paths + a concrete `docker run` example.
  - [ ] 5.2 Add reference to the user manual under a "Documentation" subsection (link to the manual file + a note about HTML/PDF build via DocBook XSL toolchain).
  - [ ] 5.3 Add an "Epic B" row to the Planning table mirroring the Epic A row structure (status, scope summary, link to the retro / manual).
  - [ ] 5.4 Append a new entry to the Current Version block on the next status transition (the workflow's standard prepend pattern).

- [ ] **Task 6 — DocBook user manual: Installation chapter (AC: #14, #18, #19)**
  - [ ] 6.1 Identify the right insertion point in `docs/manual/opcgw-user-manual.xml` (after the existing introductory `<chapter>`s; before any reference appendices).
  - [ ] 6.2 Author the `<chapter>` with `<title>Installation</title>` and the six `<sect1>` sections enumerated in AC#14.
  - [ ] 6.3 Use `<programlisting>` for `Dockerfile`, `docker-compose.yml`, `.env`, and systemd unit-file examples; use `<screen>` for shell commands; use `<envar>` for env-var names; use `<filename>` for paths.
  - [ ] 6.4 Update `docs/manual/index.xml` TOC entry for the new chapter.
  - [ ] 6.5 Validate: `xmllint --noout --valid docs/manual/opcgw-user-manual.xml` exits 0.

- [ ] **Task 7 — DocBook user manual: Configuration chapter (AC: #15, #18, #19)**
  - [ ] 7.1 Author the `<chapter>` with `<title>Configuration</title>` and the six `<sect1>` sections enumerated in AC#15.
  - [ ] 7.2 For each `[section]`, render a `<table>` listing field-name + type + default + required/optional + env-var-override + description. Use `<thead>` / `<tbody>` / `<row>` / `<entry>` per DocBook 4.5.
  - [ ] 7.3 Include at least one worked example per section (a complete `[section]` snippet in `<programlisting>` that an operator could literally paste into `config.toml`).
  - [ ] 7.4 Cross-reference `docs/security.md` (PKI, NFR9 permissions) and `docs/logging.md` (audit-event taxonomy) via `<xref>` or footnote text rather than duplicating content.
  - [ ] 7.5 Update `docs/manual/index.xml` TOC.

- [ ] **Task 8 — DocBook user manual: Troubleshooting + Upgrade chapters (AC: #16, #17, #18, #19)**
  - [ ] 8.1 Author the `<chapter>` (or `<appendix>`) for **Troubleshooting** with the seven operator scenarios from AC#16. Each scenario gets a `<sect1>` with: symptom (`<para>`), diagnostic grep recipe (`<screen>`), root-cause discussion (`<para>`), remediation (`<orderedlist>`).
  - [ ] 8.2 Author the `<chapter>` for **Upgrade and migration** referencing `docs/deployment-guide.md § "Epic A migration"`. Include both Path A and Path B as `<sect1>` sub-sections; document the one-way rollback contract.
  - [ ] 8.3 Update `docs/manual/index.xml` TOC for both new chapters.
  - [ ] 8.4 Final DTD validation pass.

- [ ] **Task 9 — Manual build pipeline (optional but recommended; AC: #19)**
  - [ ] 9.1 Add a `docs/manual/Makefile` (or shell script) that wraps the standard DocBook XSL toolchain: `xsltproc` for HTML, `fop` or `dblatex` for PDF. Reference the output directory `docs/manual/out/`.
  - [ ] 9.2 Document the build invocation in the manual's preface (`<bookinfo>` or `<preface>` `<para>`).
  - [ ] 9.3 NOT required if Guy prefers to continue building via oXygen editor (`opcgw.xpr` project). Document the choice in Dev Notes.

- [ ] **Task 10 — CHANGELOG.md + sprint-status finalization (AC: #13, #20, #21, #22)**
  - [ ] 10.1 Edit `CHANGELOG.md` `[Unreleased] — v2.0.0` Added section: add bullets for dual-registry publishing, multi-arch, Dockerfile hardening, manual chapters (Installation, Configuration, Troubleshooting, Upgrade), Docker Hub overview page.
  - [ ] 10.2 Final regression-gate: `cargo test --all-targets` (expect 1256/0/10), `cargo clippy --all-targets -- -D warnings` (expect clean), `cargo test --doc` (expect 0/55).
  - [ ] 10.3 Sprint-status flip: `B-1-docker-hub-publishing-and-user-manual: ready-for-dev → in-progress` at task-start; `→ review` at implementation-complete; `→ done` after `bmad-code-review`.
  - [ ] 10.4 README "Current Version" line gets a fresh prepended entry at the implementation-complete commit (per the existing pattern).
...
(filled in by dev agent at implementation time — concrete list of files actually touched + their commit hashes if multi-commit)

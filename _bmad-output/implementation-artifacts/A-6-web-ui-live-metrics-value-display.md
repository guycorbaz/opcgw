...
`bmad-code-review A-6` iter-2 same-LLM run completed. Iter-2 surfaced **10 HIGH-REG / MEDIUM patches** in iter-1's own patch round + 1 AC#14 documentation-sync violation — exactly matching the memory `feedback_iter3_validation` 11-story pattern's prediction (A-6 iter-2 extends the streak to 12). Acceptance Auditor iter-2 verdict: ELIGIBLE-FOR-DONE-AFTER-PATCHES (AC#5 + AC#14 documentation drift on the new `int_precision_lossy` reason was the blocker for condition #2 termination).

### patch (10, all applied 2026-05-18)

- [x] **[Review][Patch] K1 (Auditor) — AC#14 documentation-sync violation: `docs/logging.md` missing `int_precision_lossy` reason row** `[docs/logging.md:155]`. iter-1 P8 added the new reason value in code (`REASON_INT_PRECISION_LOSSY` const + emission site) but did NOT extend the `docs/logging.md` audit-event table. AC#14 first bullet requires `docs/logging.md` updated in the same commit as the behaviour change. **Fix:** rewrote the `metric_view_serialize` row to document the full closed enum `{non_finite, int_precision_lossy}` plus the per-row vs aggregate severity split, plus the K2/K3 `f32_overflow` / `f32_underflow` reasons added by iter-2 (see below).
- [x] **[Review][Patch] K2 (Blind+Edge) — P0-D1 f32 subnormal underflow produces 40-character decimal strings on the dashboard** `[src/web/api.rs:metric_type_to_json_value Float arm]`. f64 magnitudes in the f32 subnormal range (|f| < ~1.18e-38) cast to a subnormal f32; subnormal `f32::to_string()` emits decimal notation like `"0.000000000000000000000000000000000000011754942"` (45+ chars), unreadable on the dashboard. **Fix:** added an explicit `narrowed.is_normal() || narrowed == 0.0` guard; subnormal-underflow path now emits `info!(event="metric_view_serialize", reason="f32_underflow", …)` + returns `Some(0.0)` (operator sees `0 °C` instead of a 45-character cell). New reason `f32_underflow` joins the closed enum; documented in `docs/logging.md` per K1.
- [x] **[Review][Patch] K3 (Blind+Edge) — P0-D1 f32 overflow path silently emitted `debug!` instead of `warn!`** `[src/web/api.rs:metric_type_to_json_value Float arm]`. Finite f64 with `|f| > f32::MAX (~3.4e38)` narrows to `f32::INFINITY`; the old code path hit the defensive `from_f64` None arm at `debug!` level, losing operator-actionable detail at production log levels (info+). **Fix:** explicit `narrowed.is_finite()` guard upstream of the string round-trip; overflow path now emits `warn!(event="metric_view_serialize", reason="f32_overflow", device_id, metric_name, f64_value)` + returns `None`. New reason `f32_overflow` joins the closed enum.
- [x] **[Review][Patch] K4 (Blind+Edge) — P11 `metric_unit` trim silently strips NBSP (U+00A0)** `[src/web/mod.rs:DashboardConfigSnapshot::from_config]`. The iter-1 implementation used `str::trim()` which treats Unicode whitespace as trimmable, including U+00A0 NBSP. French/Spanish industrial typography routinely uses NBSP between value and unit (`metric_unit = "\u{00A0}°C"`); the trim was silently destroying the operator's intentional locale typography. **Fix:** switched to `trim_matches(|c: char| c.is_ascii_whitespace())` which only strips ASCII whitespace (space/tab/CR/LF — what TOML round-tripping introduces). NBSP and other locale-significant Unicode whitespace preserved. New unit-test case `raw_nbsp` in `dashboard_snapshot_normalises_empty_and_whitespace_unit_to_none` pins the NBSP round-trip.
- [x] **[Review][Patch] K5 (Blind+Edge) — P1+P10 empty-string handling left a hole for non-String data_type** `[static/metrics.js:renderMetricRow]`. iter-1 P1 keyed `valueMissing` off `metric.value` directly; iter-1 P10 treated `data_type === "String" && value === ""` as missing. Cross-arm hazard: a server-side wire bug (Float row with `value: ""`) would fall through both checks, hit `formatValue`'s Float arm (`"".toString() === ""`), then concatenate with the unit producing ` °C` (leading-space artifact — the original P10 bug recurs). **Fix:** simplified `valueMissing` to treat `metric.value === ""` as missing regardless of `data_type`. Defensive cross-arm collapse.
- [x] **[Review][Patch] K6 (Blind+Edge) — P12 grep guard was structurally broken** `[tests/web_dashboard.rs]`. Three independent issues: (a) regex over-matched (`assert_eq!(view.field, ...)` matches even though it's safe), (b) silently passed when `git` was absent or the cwd wasn't a git tree (exit 128 != 0), (c) missed multi-line `assert_eq!` macros (regex stops at first `)`). The grep added churn without protection. **Fix:** DELETED the grep test entirely with a code-comment explaining why the compile-time absence of `PartialEq, Eq` on these structs IS the actual guard — any future `==` between values of these types fails the build at the usage site. `serde_json::Value` blocks `Eq` outright (NaN axis), so re-adding the derive is also a compile error.
- [x] **[Review][Patch] K7 (Blind+Edge) — P3 wire-format length assertion was docstring-vs-body drift** `[src/web/api.rs::tests::metric_type_to_json_value_float_finite]`. The previous `wire.len() <= 6` check re-introduced brittleness against serde_json's stringification (the very surface P3 claimed to abandon). Worse, the magnitude tolerance `(f - 23.6_f64).abs() < 1e-5` was loose enough to pass for BOTH pre-fix code (artifact 23.600000381469727, delta 3.8e-7) AND post-fix code (exact 23.6, delta 0). A regression reverting P0-D1 would have slipped past both assertions. **Fix:** replaced both with `assert_eq!(f, 23.6_f64)` — exact equality holds post-fix (string round-trip lands on the f64-closest-to-"23.6" which IS 23.6_f64 exactly) and fails pre-fix (the f64-from-f32 artifact is `23.600000381469727 != 23.6_f64`).
- [x] **[Review][Patch] K8 (Blind+Edge) — P2 `non_finite_count: u32` arithmetic-mismatch hazard** `[src/web/api.rs::api_devices]`. The counter type was `u32` but the natural type for a per-response row count is `usize` (matches `Vec::len()` semantics, no overflow at pathological scale, no future cast hazards). **Fix:** renamed to `float_emitted_null_count: usize` (also rephrased to reflect K3's umbrella of both non_finite and f32_overflow reasons).
- [x] **[Review][Patch] K9 (Blind+Edge) — P7 `body.contains("9223372036854775807")` was path-ambiguous** `[tests/web_dashboard.rs::api_devices_emits_typed_value_and_unit_per_variant]`. The substring would match the bignum value appearing anywhere in the response body (future timestamp_ns fields, audit log lines, debug serialisations). The test claimed to pin the bignum row's `value` field but the assertion couldn't actually attribute success uniquely. **Fix:** changed to `body.contains("\"value\":9223372036854775807")` with the JSON key prefix; also added the round-trip `m["value"].as_i64() == Some(i64::MAX)` for defence-in-depth.
- [x] **[Review][Patch] K10 (Auditor) — P5 unit-presence test only pinned `unit`, not `value` or `timestamp`** `[src/web/api.rs::tests::metric_view_serialises_unit_field_even_when_none]`. AC#1's wire-contract table requires ALL three Option fields to serialise as JSON `null` (not omitted) when None. The original P5 test guarded against `#[serde(skip_serializing_if = "Option::is_none")]` only on the `unit` field; the same edit could be applied to `value` or `timestamp` and silently break the wire. **Fix:** extended the test to assert all three Option fields are present (and null) when None; renamed to `metric_view_serialises_all_option_fields_even_when_none`.

### dismiss (iter-2, 4)

- [Review][Dismiss] Blind iter-2 #7 — P1 comment claim about em-dash strings is "load-bearing dishonest". The comment is operationally accurate (em-dash strings DO get the unit suffix today); a future formatValue change that normalises Unicode dashes would break it, but that change isn't in this story. Comment polish-only, not a substantive issue.
- [Review][Dismiss] Blind iter-2 #14 — P0-D1 patch comment claims about `f32::NAN.to_string()` output. Operationally correct; minor doc cleanup.
- [Review][Dismiss] Edge iter-2 #3 — Float `-0.0` round-trip platform dependency. P3's `as_f64() + is_sign_negative()` test verified the round-trip holds; no observed platform variance.
- [Review][Dismiss] Edge iter-2 #8 — `t.len() == s.len()` byte-length comparison vs zero-width chars. Theoretical (no Unicode zero-width chars in industrial unit strings); the post-K4 ASCII-only trim makes this further unreachable.

### defer (iter-2, captured but not patching)

- [x] **[Review][Defer] DEF-iter2-A6-1 (Auditor LOW) — AC#1 / AC#5 / AC#6 spec table text not updated for the iter-1+iter-2 contract refinements** — spec text was not mutated in this iter; the Dev Agent Record / Completion Notes + this Review Findings block document the post-iter-2 contract; future spec amendment can pull these into the AC tables proper. Defer to a documentation-pass story.
- [x] **[Review][Defer] DEF-iter2-A6-2 (Auditor LOW) — P11 latent persisted-vs-snapshot whitespace drift** `[src/web/mod.rs:from_config]` — the trim normalises in-memory snapshot but does NOT mutate the persisted TOML; a future story comparing snapshot-vs-persisted would see false-positive drift. No such comparison exists today; tracking for the future story that introduces it.
- [x] **[Review][Defer] DEF-iter2-A6-3 (Auditor LOW) — P6 promised `#[test]` pinning the wire string for `REASON_NON_FINITE` not delivered** — the P4 `logs_contain("reason=\"non_finite\"")` assertions serve as de facto pins via the tracing-test capture. A dedicated string-pin test is nice-to-have but not load-bearing.
- [x] **[Review][Defer] DEF-iter2-A6-4 (Blind iter-2 #4) — aggregate warn label is "Float row(s) emitted as JSON null" but doesn't carry sample identifiers** — per-row debug! / warn! lines carry the device_id+metric_name correlation. Aggregate is a count signal; per-row is the forensics signal. Adding sample identifiers to the aggregate would duplicate the forensic surface for marginal value.
- [x] **[Review][Defer] DEF-iter2-A6-5 (Blind iter-2 #13) — P9 client-side `console.warn` per render is unthrottled** `[static/metrics.js:formatValue]` — would log-flood on a sustained forward-incompat data_type, but the structural condition is "the wire schema is ahead of the deployed JS bundle" which itself is a release-discipline problem, not an A-6 concern. Client-side throttling adds module-level state for a release-edge case.
- [x] **[Review][Defer] DEF-iter2-A6-6 (Edge iter-2 #11) — `valueMissing` future-variant coupling** `[static/metrics.js:renderMetricRow]` — a future MetricType variant (`Bytes` etc.) would need the renderer updated to recognise the new data_type; the unit suffix would render correctly today only by accident. Out of A-6 scope; tied to a hypothetical future wire-contract expansion.

### Iter-2 summary

10 patches applied (K1-K10) — 6 HIGH-REG (K1-K6), 4 MEDIUM/LOW (K7-K10). All convergent multi-source (Blind + Edge + Auditor catching the same finding-classes the iter-2-recommendation doctrine predicted). 4 dismissed. 6 deferred (all LOW; tracked for future doc-pass + release-discipline stories). Final verification: cargo test 1254 / 0 / 10 (was 1255 pre-K6; -1 from deleted P12 test); clippy clean; doctest clean. AC#5 grep returns 5 lines unchanged; AC#7 grep returns 0 hits unchanged. New closed-enum reasons `f32_overflow` and `f32_underflow` joined the `metric_view_serialize` event; `docs/logging.md` documents the full enum `{non_finite, int_precision_lossy, f32_overflow, f32_underflow}`.

**Loop termination per CLAUDE.md condition #2 (only LOW remains):** iter-2 closed all HIGH/MEDIUM/decision-needed findings. The 6 deferred items are all LOW with user-accepted rationale (3 documentation-pass deferrals, 1 latent-drift tracking for a hypothetical future story, 1 release-discipline guard, 1 future-variant coupling). The story is ELIGIBLE-FOR-DONE under condition #2.

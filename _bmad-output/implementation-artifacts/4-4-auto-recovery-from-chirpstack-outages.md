...
| `chirpstack.retry: u32` and `chirpstack.delay: u64` config knobs | `src/config.rs:121-131` + validation at `src/config.rs:931-936` | **Wired today.** Both validated as `> 0`. FR8 contract. Default values in `config/config.toml` were `retry = 1, delay = 1` at spec-write time but had drifted to `retry = 10, delay = 10` by review iter-3 (2026-05-06); patch P14 tightened them to `retry = 30, delay = 1` to satisfy AC#4's `retry × delay ≤ 30s` verification clause. 4-4 reads these for the recovery envelope; do not change validation. |
...
- **And** the 30-second budget assumes default-ish config (`retry = 30`, `delay = 1`, polling_frequency ≤ 30); operators may tune `chirpstack.retry` / `chirpstack.delay` to widen or narrow this. **The default `config/config.toml` MUST keep the NFR17 budget achievable** — verify the shipped defaults satisfy `retry × delay ≤ 30s`. Iter-3 review patch P14 (2026-05-06) set the shipped defaults to `retry = 30, delay = 1` (worst-case 30s budget) to satisfy this clause.
...
- **Then** the following docs are updated as part of the implementation commit:
  ...
  - **`config/config.toml`** — **CHANGED (review iter-3 patch P14, 2026-05-06)**. The `chirpstack.retry` and `chirpstack.delay` knobs already existed but the shipped defaults had drifted to `retry=10, delay=10` (worst-case 100s budget — violated AC#4's `retry × delay ≤ 30s` clause). P14 reset to `retry=30, delay=1` and added a doc-comment block above the knobs documenting the AC#4 invariant.
...
**Iter-1 review run on 2026-05-05** via `bmad-code-review` (same Opus 4.7 session as implementation — same-LLM caveat acknowledged; per CLAUDE.md "Code Review & Story Validation Loop Discipline" preference for cross-LLM review explicitly waived per Guy's over-reviewing > under-reviewing standing preference, with reviewers instructed to be especially aggressive). Three parallel adversarial layers (Blind Hunter / Edge Case Hunter / Acceptance Auditor) produced 13 + 21 + 12-AC audit + 5 concerns = ~50 raw findings → ~25 unique after dedup.

**Same-LLM caveat paid off — three real HIGH bugs surfaced that the implementation rationalised away:**

1. Cancel `attempts_used` off-by-one (log says `attempt=1`, return says `attempts_used=0`).
2. Sleep BEFORE first probe wastes recovery budget (NFR17 burned on cold-restart even when server is already back).
3. Channel-connect 5s timeout returns `OpcGwError::Configuration` — but wire-in's `matches!(e, OpcGwError::ChirpStack(_))` rejects it, so the recovery loop wouldn't fire on channel timeouts.

Triage: 1 decision-needed (D1 — integration test gap), 9 patches (P1-P9 after D1 conversion), ~14 deferred, ~3 dismissed.

#### Decision-needed (resolved)

- [x] [Review][Decision] **D1: Integration test gap — Auditor MED-HIGH.** Spec AC#12 mandated `3-5 unit AND 3-5 integration` tests. Implementation: 0 integration tests, 0 new file. Auditor: rationale "field-shape divergence (functionally equivalent)" is wrong — wire-in at `chirpstack.rs:1224-1243` contains four load-bearing contracts (just_logged-gating, single-fire-per-cycle, error_count post-saturating-add semantics, no-cycle-break) that the 7 unit tests don't pin. Resolved 2026-05-05: Option 1 (patch with `poll_metrics`-driving integration test). Becomes patch **P9** below.

#### Patches (9, all applied 2026-05-05)

- [x] [Review][Patch] **P1 [HIGH]: Restructure recovery loop — probe BEFORE sleep + fix cancel `attempts_used` off-by-one.** Two findings combined: (a) F1/EH4 cancel-during-first-sleep returns `attempts_used: 0` while a `recovery_attempt` log emitted `attempt = 1` (log/return mismatch); (b) F2 sleep-before-probe forces operator to wait `delay` seconds for first probe even when server is already back (burns NFR17 budget). Fix: probe FIRST on each iteration, sleep ONLY between attempts (skip after final attempt). Cancel during inter-attempt sleep returns `attempts_used: attempt_num` (the attempts that DID complete). [`src/chirpstack.rs:858-924`]

- [x] [Review][Patch] **P2 [HIGH]: Channel-connect 5s timeout returns `OpcGwError::ChirpStack` (not `Configuration`).** Wire-in's `matches!(e, OpcGwError::ChirpStack(_))` check at `src/chirpstack.rs:1228` rejects `Configuration` variant, meaning a slow ChirpStack failing the 5s timeout would NOT trigger the recovery loop. Pre-existing inconsistency in the existing `Err(e)` arm too — fixed in this patch alongside the new timeout path for shape consistency. [`src/chirpstack.rs:528-547`]

- [x] [Review][Patch] **P3 [MED]: Cancel branch preserves original outage cause in `last_error`.** Previous code: `last_error: "cancelled during recovery wait".to_string()` — discards the actual gRPC failure that triggered the recovery loop. Fix: include the original `last_error` parameter's display in the cancel-branch message: `format!("cancelled during recovery wait; outage cause: {}", last_error)`. Helps post-mortem triage. [`src/chirpstack.rs:881-887`]

- [x] [Review][Patch] **P4 [LOW]: Tighten test assertions.** (a) `recovery_aborts_on_cancel_during_sleep` now asserts `attempts_used` explicitly (was missing per F6 — combines with P1's bug). (b) `recovery_complete_carries_downtime_secs` substring `=10`/`=11` matched `=100`/`=110` too — replaced with regex-anchored or exact-line check. (c) `recovery_complete_emits_from_startup_when_no_prior_poll` negative substring `!logs_contain("downtime_secs")` is brittle (any log line elsewhere mentioning the substring would false-fail) — replaced with positive shape assertion on the recovery_complete line.

- [x] [Review][Patch] **P5 [LOW]: Update `docs/logging.md` "Related stories" stale text.** Line ~243 still said `**Story 4-4** (planned) — auto-recovery from ChirpStack outages; will use the operations reserved above.` — but Story 4-4 is no longer "planned" (status=review) and the "reserved above" subsection was removed. Updated to `(implemented 2026-05-05) — auto-recovery from ChirpStack outages; defines recovery_attempt / recovery_complete / recovery_failed operations.`

- [x] [Review][Patch] **P6 [LOW]: Update doc-comment line-number reference in `recover_from_chirpstack_outage`.** Doc-comment cited `docs/logging.md:240-242` (the old reserved subsection); after promotion the operations are at `docs/logging.md:148-150`. Updated to remove the line-number citation entirely (just names the operations) — line numbers drift over time and the operation names are stable.

- [x] [Review][Patch] **P7 [LOW]: Strengthen AC#3 round-trip test with pre-seeded `last_poll_ts`.** `recovery_updates_gateway_status_to_unavailable` asserts `last_poll_ts.is_none()` against a fresh `InMemoryBackend` — passes trivially. A regression that passed `Utc::now()` instead of `None` to `update_gateway_status` would not be caught. Added pre-seed step: write `Some(t1)` first, then assert `Some(t1)` preserved after recovery — actually exercises the trait's `None`-preserves-existing contract.

- [x] [Review][Patch] **P8 [LOW]: Strengthen `recovery_attempt` operator-action in `docs/logging.md`.** Was wishy-washy ("Monitor frequency. A spike of recovery_attempt lines correlates with a chirpstack_outage warn — investigate ChirpStack server health.") — made concrete: "Expected during ChirpStack outage; if frequency is high during normal operation (>1/hour without preceding chirpstack_outage), check `chirpstack.delay` config and upstream health."

- [x] [Review][Patch] **P9 [MED-HIGH]: Add wire-in contract test (partial resolution of D1).** Original design wanted a `poll_metrics`-driving end-to-end integration test, but that approach hit a dead end: tonic's `Channel::connect()` returns successfully once the kernel SYN-ACK completes (well before HTTP/2 SETTINGS), so the P2 5s channel timeout doesn't bound the request-level hang against a stub TCP listener. A true end-to-end integration test would require either a mock gRPC server (~200+ LOC of test infrastructure) or a request-level timeout in `get_device_metrics_from_server` (out of Story 4-4 scope — the deferred-work.md:86 6-3 carry-forward was specifically about `Channel::connect`, not gRPC requests). **Pivoted to a contract-pinning unit test** `recovery_wire_in_just_logged_gating_single_fires_per_cycle` that exercises the same call sequence the wire-in performs: 3 simulated per-device ChirpStack errors → asserts `just_logged == true` only on the first call, `false` on calls 2 and 3 (single-fire-per-cycle contract); asserts `recovery_attempt` + `recovery_complete` fire once in the cycle (not three times); structurally pins the wire-in's just_logged-gating + once-per-cycle contracts. The broader integration test coverage (poll_metrics end-to-end with mock gRPC) is deferred to `deferred-work.md` under "Story 4-4 review iter-1 deferrals" — see entry there for the test-infrastructure rationale.

#### Deferred (14)

- [x] [Review][Defer] **Cumulative `Channel::connect` 5s × N-devices blows NFR17** [`src/chirpstack.rs` create_channel] — F8/EH13. Pre-existing pattern: each device call rebuilds its own channel. With N=10 devices and a slow upstream, R × 5s = 50s could blow NFR17's 30s SLA at the cycle level. NFR17 measures recovery-loop budget (R × delay) not full-cycle latency, so this is a tangential concern. Defer to Story 9-7 hot-reload work or a separate channel-pooling story when SCADA-side throughput pain surfaces.
- [x] [Review][Defer] **Inner probe-loop in `get_device_metrics_from_server` is not cancel-safe** [`src/chirpstack.rs:1786-1847`] — EH5. Pre-existing pattern: the intra-cycle TCP probe retry loop sleeps `delay` between attempts but doesn't `tokio::select!` against `cancel_token`. A long-running poll cycle could delay graceful shutdown by R × delay seconds. Pre-existing concern, not 4-4 regression.
- [x] [Review][Defer] **`#[serial_test::serial]` discipline on the 7 new tests** [`src/chirpstack.rs::tests`] — EH17. Existing `chirpstack.rs::tests` don't use `#[serial_test::serial]` either; `tracing-test` global-buffer cross-pollination is theoretically possible but hasn't been observed. Defer until a flake actually surfaces.
- [x] [Review][Defer] **Port-rebind race in unit-test fixture** [`src/chirpstack.rs::tests` recovery_*] — F13/EH16. `bind('127.0.0.1:0')` + `drop` for the unbound case has a narrow race window where another process could grab the port. Pre-existing pattern from Story 8-1 / 9-0; never observed in CI.
- [x] [Review][Defer] **Recovery success doesn't reset `chirpstack_available=true`** [`src/chirpstack.rs:888-905`] — EH8. Spec explicitly says don't (avoid race with cycle-end write at `poll_metrics:1188-1195`). Documented design choice.
- [x] [Review][Defer] **`RecoveryOutcome` discarded by caller via `let _outcome`** [`src/chirpstack.rs:1235-1244`] — F3/EH11. Spec wire-in instruction explicitly says "control returns to the device for-loop... Do not break the cycle early." Returned enum is informational; documented design choice.
- [x] [Review][Defer] **`retry == 0` defense-in-depth** [`src/chirpstack.rs` recover_from_chirpstack_outage] — F11/EH3. Config validation rejects `retry == 0` at startup (`src/config.rs:931-936`). Defense for a future caller that bypasses validation; not a present bug.
- [x] [Review][Defer] **`error_count_at_entry: i32` could be `u32`** [`src/chirpstack.rs` recover_from_chirpstack_outage signature] — F12. Style; `update_gateway_status` accepts `i32` historically. Refactor when storage trait is rewritten (Epic-A storage payload migration).
- [x] [Review][Defer] **`update_gateway_status` entry-write Err is logged warn but proceeds** [`src/chirpstack.rs:846-854`] — EH9. Defensive choice: storage failure during outage is a secondary failure; logging warn + proceeding with the recovery probe is the right call. Documented in code comment.
- [x] [Review][Defer] **Race between recovery's `update_gateway_status` write and cycle-end write** [`src/chirpstack.rs` recover_* + poll_metrics:1188-1195] — EH10. Eventually consistent: the cycle-end write is the authoritative state for the next cycle. Defer until SCADA-side observability gap surfaces.
- [x] [Review][Defer] **Cancel fired BEFORE recover_from_chirpstack_outage entry** [`src/chirpstack.rs` recover_from_chirpstack_outage] — EH19. Defensive guard would emit no `recovery_attempt` and skip the gateway_status write. Rare path; the cancel will be observed at the first `tokio::select!` inside the loop instead.
- [x] [Review][Defer] **Originating device not retried after Recovered outcome** [`src/chirpstack.rs:1234-1243`] — EH20. Spec design choice: subsequent devices in the same cycle benefit from the restored channel; the originating device gets refetched on the next cycle (`polling_frequency` later — typically 10s). Acceptable user-visible gap.
- [x] [Review][Defer] **NFR17 30s SLA has no regression test** [`src/chirpstack.rs::tests`] — Auditor concern #5. Spec called for `test_nfr17_recovery_within_30_seconds`; implementation assumes shipped defaults trivially satisfy. A future bump that breaks the SLA wouldn't be caught by `cargo test`. Defer; tracked as a hardening follow-up.
- [x] [Review][Defer] **Test-fixture comment `// Bypasses validation (delay=0 would fail validate())` is misleading** [`src/chirpstack.rs::tests::make_recovery_test_poller` line ~2935] — Auditor divergence #4. `ChirpstackPoller::new()` doesn't actually call `validate()`; comment is functionally wrong but harmless. Cosmetic.

#### Dismissed (3)

- F3 (Cold-start `last_error` field — Blind Hunter retracted on reread).
- EH7 (Clock-skew negative duration → silently 0) — code uses `.max(0) as u64` which correctly handles negative `signed_duration_since`. False positive.
- F13 (5s timeout per call — single-call vs cumulative) — subsumed by F8 cumulative concern (deferred above); the per-call 5s budget itself is correct.

---

#### Iter-3 Review (2026-05-06)

User requested a fresh `bmad-code-review` pass after iter-2 PASS, to satisfy CLAUDE.md "Code Review Loop Discipline" preference for over-reviewing. Three parallel adversarial layers (Blind Hunter / Edge Case Hunter / Acceptance Auditor) on the full final state (commit `673f54b` + uncommitted iter-1 patches). Iter-3 surfaced **6 NEW findings** plus 4 re-flags of already-deferred items.

##### Decision-needed (1)

- [x] [Review][Decision] **D2 [HIGH]: Shipped `config/config.toml` defaults `retry=10, delay=10` violate AC#4 verification clause** — RESOLVED 2026-05-06 via Option (a): tightened shipped defaults to `retry = 30, delay = 1` (matches AC#4 line 144 example verbatim). Worst-case recovery budget is now ~30s; AC#4 verification clause `retry × delay ≤ 30` satisfied (30 × 1 = 30). Doc-comment in `config/config.toml:59-65` documents the AC#4 invariant for future operators tuning the knobs. Resolution becomes patch **P14** below.

##### Patches (5)

- [x] [Review][Patch] **P14 [HIGH]: Tighten shipped `config/config.toml` defaults from `retry=10, delay=10` (worst-case 100s) to `retry=30, delay=1` (worst-case 30s) — D2 resolution.** Satisfies AC#4 verification clause `retry × delay ≤ 30`. Added doc-comment block above the knobs explaining the invariant: "AC#4 / NFR17: shipped defaults must satisfy `retry × delay ≤ 30s` so the auto-recovery loop stays within the 30s SLA. Operators tuning these knobs should preserve that invariant." Knob description corrected: "Number of recovery probe attempts when ChirpStack is unreachable. Note: this is an attempt count, not a retry count — `retry = 1` means one probe before giving up (zero retries after the original failure)." [`config/config.toml:57-65`]

- [ ] [Review][Patch] **P10 [MED]: Cancel-branch + `Exhausted::last_error` lose original gRPC outage cause after first failed probe.** Patch P3's spec text (iter-1 line 355) called for `format!("cancelled during recovery wait; outage cause: {}", last_error)` — preserving the original `last_error` parameter. Actual code at `src/chirpstack.rs:937-940` uses `last_attempt_error` ("last probe error: ..."), which is overwritten on the first failed probe (line 918) — losing the original gRPC outage cause that triggered the recovery loop. Same loss in `RecoveryOutcome::Exhausted::last_error` (line 957). Fix: introduce a separate `original_outage_cause: String` (or `&str` borrow of `last_error`) that is never overwritten, and surface it in both the cancel-branch message and the `Exhausted` return. [`src/chirpstack.rs:866`, `:917-919`, `:937-940`, `:957`]

- [ ] [Review][Patch] **P11 [MED]: `chirpstack.retry=N` means N attempts (not N retries) — clarify semantics in config + log fields.** Operators tuning `chirpstack.retry` may expect "N retries after the original failure" but get "N total attempts" (one probe per increment, no inter-attempt sleep when N=1). Log labels `attempt=1, max_retries=1` reinforce the confusion when followed by an immediate `recovery_failed`. Fix: (a) add a doc-comment in `config/config.toml` near the `retry` knob clarifying "Number of recovery probe attempts (not retries; retry=1 = one probe before giving up)"; (b) consider renaming the log field `max_retries` → `max_attempts` for clarity, or document the equivalence in `docs/logging.md`. [`config/config.toml:60`, `src/chirpstack.rs:880`]

- [ ] [Review][Patch] **P12 [LOW]: `chirpstack_outage` operator-action text contradicts shipped 10/10 defaults.** `docs/logging.md` operator-action for the `chirpstack_outage` warn says "the recovery loop has been failing — check `recovery_failed` warns" — but with shipped `retry=10, delay=10`, `chirpstack_outage` and `recovery_failed` warns are emitted ~100s apart per outage cycle, not co-emitted. The runbook should clarify the temporal relationship. Aligns with D2 outcome — re-text once defaults are settled.

- [ ] [Review][Patch] **P13 [LOW]: `recovery_complete_carries_downtime_secs` assertion field-order coupled.** `logs_contain("downtime_secs=10 ")` (trailing space) disambiguates `=10` from `=100`/`=110` only because the current macro emits `downtime_secs` followed by `last_error`. A future field-order swap or formatter change could silently break the assertion. Fix: switch to a regex anchor (`\bdowntime_secs=1[0-2]\b`) or assert the structured `tracing::field::Visit` shape directly. [`src/chirpstack.rs:3209-3211`]

##### Deferred (1)

- [x] [Review][Defer] **Cancel-during-probe path untested; existing `recovery_aborts_on_cancel_during_sleep` is timing-flaky on slow CI** [`src/chirpstack.rs:3289-3306`] — Blind M5. Test relies on a 50ms cancel landing during the first inter-attempt sleep, but probe-before-sleep semantics (P1) plus a fast loopback connect-refused path (microseconds) mean the cancel almost always lands during sleep on dev hardware. On slow CI runners (>50ms connect-refused), the cancel would land during the synchronous probe, which isn't observable until the next `tokio::select!` — at which point the assertion `elapsed < 2s` may fail. Defer to a hardening follow-up; CI flake hasn't been observed yet. Tracked in `deferred-work.md`.

##### Re-flags of already-deferred items (no-ops)

The following Blind Hunter and Edge Case Hunter findings re-surfaced concerns already deferred in iter-1; no new information warrants reopening them:

- **Blind H1 / EH13**: Cumulative `Channel::connect` 5s × N-devices on cycle continuation after `Exhausted` — already deferred (line 371). Sharper analysis appreciated; classification unchanged.
- **Blind H2 / EH10**: `error_count` mid-cycle write overwritten by cycle-end Story 5-3 write — already deferred (line 380). Operator-visibility lens noted; defer stands.
- **Edge M-EC1**: Sync `TcpStream::connect_timeout` blocks cancellation; recovery loop amplifies — already deferred as "Inner probe-loop in `get_device_metrics_from_server` is not cancel-safe" (line 372). Classification unchanged.
- **Edge M-EC2**: `chirpstack_available` not flipped back to `true` mid-cycle on `Recovered` — already deferred as a documented design choice (line 375). Classification unchanged.

##### Dismissed (1)

- **Auditor AC#11 (README + sprint-status absent)** — VERIFIED FALSE POSITIVE. `README.md` line 207 has Story 4-4 row; `sprint-status.yaml` was updated in commit `673f54b`. The "Partial" status was an artifact of the narrowed iter-3 diff scope (4 files), not a real gap.

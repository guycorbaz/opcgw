...
### Review Findings (iter-2, 2026-06-11 — re-review of the iter-1 patch round, 3 layers on Opus)

- [x] [Review][Patch] **MEDIUM (blind+edge converged): `ok().flatten()` in `filter_fresher_writes` swallowed storage read errors — the freshness guard silently disabled itself exactly under storage contention** [src/chirpstack_events.rs] — FIXED: fail-closed (skip the unverifiable write; next event retries) + new `uplink_guard_read_failed` warn + logging.md row.
- [x] [Review][Patch] **MEDIUM (blind+edge converged): cross-application dedup was first-app-wins-all — a metric only the second application maps was silently never streamed** [src/chirpstack_events.rs `streamed_devices`] — FIXED: metric lists MERGE (per-metric first-occurrence wins on `chirpstack_metric_name` conflicts); test rewritten with divergent lists (`streamed_devices_dedups_and_merges_cross_application_deveui`).
- [x] [Review][Patch] **LOW (edge): `backfill_device` not cancellation-aware — shutdown could wait on the bounded fetch's own connect** [src/chirpstack_events.rs `connect_and_stream`] — FIXED: wrapped in `tokio::select!` with the cancel token.
- [x] [Review][Patch] **LOW (blind): replay-guard test lacked a positive consumption signal (could pass if the replayed event were never pumped)** [src/chirpstack_events.rs tests] — FIXED: `next_event_calls` counter on the scripted stream; the test now waits for the pump's 3rd call (proof item 2 was fully ingested) before asserting.
- [x] [Review][Patch] **LOW (blind): `unparseable_body` drop logged zero context** [src/chirpstack_events.rs `parse_up_event`] — FIXED: `body_len` field (numeric, injection-safe).
- [x] [Review][Patch] **LOW (blind): negative DB threshold coerced to None silently — divergent from the config path's hard error** [src/storage/sqlite.rs load] — FIXED: `storage_invalid_stale_threshold` warn + logging.md row (incl. repair SQL).
- [x] [Review][Patch] **LOW (auditor): `uplink_replay_skipped` doc row omitted the `device_id` field** [docs/logging.md] — FIXED: field schema completed.
- [x] [Review][Patch] **LOW (auditor): "backfill is in this diff" dismissal wording imprecise (backfill landed in `c18d289`, not the iter-1 commit)** [story Change Log] — FIXED: wording corrected; the dismissal substance was verified correct.

Acceptance Auditor iter-2 verdict: **ELIGIBLE-FOR-DONE-PENDING-AC11** — all 9 iter-1 patches verified genuinely delivered; 3 deferrals recorded; AC#2/#3/#7/#9 not regressed; third AA finding (uplink_event_dropped row "honest as written") informational, no action.

Dismissed iter-2 (7, with evidence): BH "TOCTOU between pump and backfill" (they are sequential awaits within the single per-device task; the poll never writes streamed devices — no concurrent same-device writer exists); BH "equal-timestamp winner undocumented/untested" (documented on `is_fresher` — equal → skip, no churn — and pinned by `is_fresher_guard_boundaries`; replayed duplicates carry identical bodies); BH "2^53 boundary `<=` off-by-one" (ECH verified: 2^53 is exactly representable, accepted correctly); BH "-0.0 / Bool-from-1.0 asymmetry" (ECH verified -0.0/NaN/Inf all correctly handled; flags are integer-contracted per codec); BH "select-cancel drops in-flight connect / Ok(()) ambiguous" (intended shutdown semantics, same contract as the pump-loop cancel arm; transport cleans up); BH "verify schema messages consistency" (grep-verified all six updated, v006→v012 chain message corrected); ECH "empty candidates list" (verified handled — no log, no write).
...
| 2026-06-11 | **Code review iter-2** (re-review of the iter-1 patch round, 3 layers on Opus): 8 patches — 2 MEDIUM converged BH+ECH **in iter-1's own new code** (the iter-N+1 doctrine firing again: `ok().flatten()` silently disabled the freshness guard on storage read errors → fail-closed + `uplink_guard_read_failed`; first-app-wins dedup silently dropped the 2nd application's distinct metrics → per-metric MERGE) + 4 LOW code (cancellable backfill, replay-test positive consumption signal via next_event counter, `body_len` on unparseable_body, `storage_invalid_stale_threshold` warn) + 2 LOW doc (AA: `uplink_replay_skipped` missing `device_id` field, changelog wording). 7 dismissed with evidence (TOCTOU refuted — backfill/pump sequential per device task; equal-ts documented+tested; 2^53/-0.0/NaN verified correct by ECH). AA verdict: **ELIGIBLE-FOR-DONE-PENDING-AC11**, all 9 iter-1 patches verified genuinely delivered. Tests 16/16 module, full suite 35 suites green, clippy clean. Iter-3 launched (iter-2 introduced new flow-control: fail-closed branch + merge logic + counter fixture). |
| 2026-06-11 | **Code review iter-1** (3 parallel layers on Opus; impl model Fable 5): 9 patches applied — headline = freshness guard extended to the LIVE pump (`filter_fresher_writes` shared with backfill; ChirpStack replays event history on every (re)connect, which previously regressed last-values transiently), cross-application DevEUI stream dedup, per-device threshold validation + negative-load guard, strict 0/1 Bool + integral-only Int coercion, explicit orphan-tracking statements, `uplink_event_dropped` diagnostic, v012 assertion-message fixes, cancellation-aware connect. +4 tests (lib module 16/16). 3 LOW defers in deferred-work.md, 8 dismissed (incl. Blind Hunter's false HIGH on `update_device` — verified the UPDATE preserves the threshold column). Dismissed Blind Hunter "no backfill" finding — backfill is present in the code under review (it landed in the story's E-1b implementation commit `c18d289`, part of the reviewed story diff). Iter-2 re-review MANDATORY per the iter-N+1 doctrine (iter-1 introduced new flow-control: live-path guard + dedup + cancellable connect). |

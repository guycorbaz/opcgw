...
- **Story 9.8 (dynamic OPC UA address-space mutation) likely needs its own spike.** async-opcua 0.17.1's `SimpleNodeManagerImpl` is not designed for runtime mutation of an already-running server's address space. The contract under live subscriptions / `SyncSampler`-driven sampling is unclear; `SimpleNodeManagerImpl::add_node` exists but has not been validated against opcgw's address-space construction shape. **Recommended sequencing: introduce a Story 9-X spike** (analogous to Story 8-1's role for subscriptions) **OR** front-load Story 9.8's first acceptance-criteria pass with a "Library Audit" subsection that empirically validates `SimpleNodeManagerImpl::add_node` / `remove_node` semantics under live subscriptions before any Story 9.8 implementation work begins. Without this, Story 9.8 risks hitting a Plan B wall mid-implementation.

...
| `metric_read` | `warn` | A-4 (FR51): an OPC UA Read produced an f64-payload that narrowed to a non-finite f32 (the f64 value is finite but its magnitude exceeds `f32::MAX ≈ 3.4×10³⁸`). **Field schema (closed enum):** `event="metric_read"`, `device_id=<str>`, `metric_name=<str>`, `f64_value=<f64>`, `reason ∈ {narrowing_overflow}`. The gateway returns `Variant::Float(0.0)` to the SCADA client rather than `Inf`/`NaN`. | Investigate the source — most field sensors don't legitimately emit values beyond f32 range. If the source is legitimate (rare scientific measurement), expose the metric via Variant::Double rather than Variant::Float. |

...
| `apply_failed` | `warn` (audit) | F-0 | The supervisor could not re-read the configuration from SQLite on Apply (figment/validation error). Field: `error=<str>`. **Non-disruptive** — the re-read happens *before* teardown, so the current data-plane keeps running unchanged and the operator can retry. |

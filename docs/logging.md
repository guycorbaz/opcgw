...
| `storage_init` | `info` (audit) / `warn` for wider-than-0o600 mode | D-0 | SQLite file-permission diagnostics (AI-C-SEC-2 hardening). `info` fires on fresh creation of `data/opcgw.db` when opcgw successfully chmod's the file to 0o600; fields `path=<str>`. `warn` fires once-per-boot on an existing database whose file mode is wider than 0o600 (e.g. inherited 0o644 from a 0o022 umask deployment); fields `path=<str>`, `mode=<oct>`. The warn is non-fatal — the gateway continues normally; the runbook (`docs/d-0-migration-runbook.md`) documents the chmod recipe. Windows deployments use ACLs rather than POSIX mode bits and do not emit this event. |

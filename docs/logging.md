...
| `inventory_query_failed` | `warn` | Story C-1: a `/api/inventory/*` query failed at the ChirpStack call. **Field schema:** `event="inventory_query_failed"`, `resource ∈ {applications, devices, uplinks}`, `reason ∈ {chirpstack_unreachable, chirpstack_auth_failed, chirpstack_grpc_error}`, `tenant_id=<str>`, `application_id=<str when applicable>`, `dev_eui=<str when applicable>`, `error=<str>`, `duration_ms=<u64>`. Returns HTTP 502 to the client. | `chirpstack_unreachable` → check ChirpStack reachability (same triage as `chirpstack_outage`). `chirpstack_auth_failed` → check `[chirpstack].api_token` and the token's tenant scope. `chirpstack_grpc_error` → inspect the `error=` field for the gRPC status code. |

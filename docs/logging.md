...
| `command_dispatch_drain_error` | `warn` | Story J-1 (CR #136): the dispatcher could not read the pending-command queue for this drain (storage-lock / backend error). **Fields:** `error=<str>`. The error is logged and swallowed — the long-lived dispatcher task is **not** aborted; the drain retries on the next dispatch signal, so a transient storage fault delays but does not strand commands. | One-off under contention is benign. Persistent → storage fault; same triage as other storage errors. Commands stay `Pending` and are re-attempted on the next write-driven wakeup. |

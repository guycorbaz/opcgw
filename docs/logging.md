...
- **Story D-1** — Singleton config editor UI. Adds six new audit events: `config_overlay` (info — boot-time SQLite overlay applied to AppConfig), `config_overlay_failed` (warn — overlay error, fall back to figment), `config_get_singleton` (info — GET request served), `singleton_config_updated` (info — PUT persisted to SQLite, fields `section` + `field_count` + `auth_user`, **no per-field VALUES logged** per the operator-data-protection design call), `singleton_config_rejected` (warn — PUT rejected with closed-enum `reason` ∈ {`validation` / `secret_field_not_editable` / `invalid_section` / `csrf` / `reload_failed`}), `singleton_config_restart_required` (info — fires immediately before `state.shutdown_token.cancel()` to record restart intent even if the process is killed mid-restart).

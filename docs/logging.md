...
| `command_dispatch_retry` | `warn` | Story J-1 iter-3 (D1): a command's ChirpStack enqueue failed **transiently** (gRPC connect/enqueue error, RPC deadline) — the row is left `Pending` and the dispatcher re-drives it on the escalating backoff, bounded by the delivery deadline (`global.command_delivery_timeout_secs` from `created_at`). **Fields:** `error=<str>`, `device_id`, `command_id`. Pre-iter-3 behaviour marked these terminally `Failed` — a 5 s ChirpStack restart permanently failed the command. Mapping failures are still immediately terminal (they can never succeed). | A short burst during a ChirpStack restart/boot is expected and self-heals. Attempts are bounded: once the deadline passes, the command fails with `command_dispatch_expired`. Persistent across many commands → ChirpStack unreachable; check network/`chirpstack_connect`. |
| `command_dispatch_expired` | `warn` | Story J-1 iter-3 (D2): a `Pending` command exceeded the delivery deadline (`global.command_delivery_timeout_secs`, default 60 s, measured from `created_at`) before it could be enqueued — marked `Failed`, **never delivered**. This is the safety gate that keeps a stale command (e.g. one carried across hours of downtime to a later boot's startup drain) from actuating hardware long after the operator wrote it. **Fields:** `command_id`, `device_id`, `age_secs`, `deadline_secs`. | Expected after any outage that outlived the deadline while commands were queued. The operator re-issues the command if still wanted. Frequent expiries *without* an outage → ChirpStack persistently unreachable (see `command_dispatch_retry`). |

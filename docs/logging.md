...
| `uplink_guard_read_failed` | `warn` | Story E-1 (review iter-2): the freshness guard could not read the stored timestamp for a metric (`device_id`, `metric`, `error`) and skipped the write **fail-closed** — an unverifiable candidate must not risk regressing the last-value; the next live event retries. | One-off under storage contention is benign. Persistent → storage fault; same triage as `uplink_store_failed`. |
| `storage_invalid_stale_threshold` | `warn` | Story E-1 (review iter-2): a negative per-device `stale_threshold_seconds` was found in the `devices` table at load (only possible via hand-edited DB — the config path rejects it in `validate()`); the global threshold is used instead (`value=<i64>`). | Fix the row: `UPDATE devices SET stale_threshold_seconds = NULL WHERE stale_threshold_seconds < 0;` |
...
| `singleton_config_restart_required` | `info` (audit) | D-1 | Fires immediately before `state.shutdown_token.cancel()` to record restart intent even if the process is killed mid-restart. |

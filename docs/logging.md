...
| `uplink_replay_skipped` | `debug` | Story E-1 (review iter-1): live-stream events not fresher than the stored last-value were skipped (`skipped=<n>`). Expected on every stream (re)connect — ChirpStack replays recent event history, and the freshness guard keeps the value path monotonic by device-report time. | None — the guard working as intended. Frequent occurrences outside reconnects would suggest out-of-order delivery upstream. |

...
| `uplink_field_type_mismatch` | `warn` (first) / `debug` (repeats) | Story E-1 (E-1a), dedup added in Story J-0 (#160): a decoded uplink field could not convert to the configured `metric_type` and was skipped. Fields: `device_id`, `metric=<chirpstack_metric_name>`, `configured_type`. **Emitted at `warn` only the FIRST time for a given (device, metric); later occurrences drop to `debug`** so a persistently mistyped field cannot flood the log. Also recorded once to the web error feed as category `metric_type_mismatch` (visible at `/errors.html`). The reconnect-backfill path logs at `debug` with `source="backfill"` and records nothing (it re-processes the same event on every connect). The dedup is per stream task, so it re-arms after an **Apply changes** soft restart — fix the type and a still-broken config warns again. | Align the device's `read_metric.metric_type` with the codec's field type (e.g. String `state` vs numeric). |
| `uplink_metric_never_seen` | `warn` | Story E-1 (E-1b): after the first few uplinks (`events_observed`), a configured `read_metric` (`metric=<chirpstack_metric_name>`) has not yet appeared in the device's decoded object. Once per field per device; since Story J-0 (#160) it is also recorded once to the web error feed as category `metric_never_seen` (visible at `/errors.html`). **Not necessarily a permanent orphan** — if the field is just intermittent/late it will arrive later and a sibling `uplink_metric_now_seen` (info) follows. | If it's followed by `uplink_metric_now_seen`, the field is conditionally-emitted — benign. If it never resolves: the metric is DevStatus-sourced (e.g. battery) not in the uplink object, OR `chirpstack_metric_name` doesn't match the codec's field name. Fix the config field name, get the codec to emit it, or accept it can't stream. Key signal during the `stream_all_devices` fleet migration. |
...
| `command_dispatch_drain` | `debug` | Story J-1 (CR #136): the event-driven `CommandDispatcher` woke and is delivering the currently-`Pending` downlink commands. **Fields:** `count=<n>` (rows in this drain). Fires once per non-empty drain — a drain finding nothing pending emits no line. The dispatcher is woken by the OPC UA write path (a successful `set_command` fires a `tokio::sync::Notify`), so a command reaches ChirpStack within seconds of the write instead of waiting up to a full `chirpstack.polling_frequency` cycle. Command delivery is **no longer** part of the metrics poll. | None — normal command delivery. A `count` that climbs across successive drains without the queue emptying → check `command_dispatch_drain_error` and the sink/gRPC health. |
| `command_dispatch_drain_error` | `warn` | Story J-1 (CR #136): the dispatcher could not read the pending-command queue for this drain (storage-lock / backend error). **Fields:** `error=<str>`. The error is logged and swallowed — the long-lived dispatcher task is **not** aborted; the dispatcher **self-retries** on an escalating backoff timer (`2s→4s→…→60s` cap, J-1 review P1/P5 — it does *not* wait for the next write-driven signal), so a transient storage fault delays but does not strand commands. | One-off under contention is benign. Persistent → storage fault; same triage as other storage errors. Commands stay `Pending` and are re-attempted on the self-driven backoff (or sooner, on the next write-driven wakeup). |
| `command_dispatch_retry` | `warn` | Story J-1 iter-3 (D1), narrowed iter-4: ChirpStack was **provably unreachable before any send** (channel/TCP connect failed — restart, boot ordering) — the row is left `Pending`, the drain **short-circuits** (remaining rows deferred, one connect attempt per drive, not one per row), and the dispatcher re-drives on the escalating backoff, bounded by the dispatch deadline (`global.command_dispatch_deadline_secs` from `created_at`). **Fields:** `error=<str>`, `device_id`, `command_id`. Only this class retries: an **ambiguous** failure (RPC sent but errored/timed out — ChirpStack may have committed the item) is terminal `Failed` with reason "delivery uncertain — verify the device queue in ChirpStack before re-issuing", because retrying it could enqueue the same downlink twice. Pre-iter-3 even connect failures were terminal; iter-3 briefly retried everything (double-send hazard); iter-4 is the safe middle. Mapping failures are immediately terminal. | A short burst during a ChirpStack restart/boot is expected and self-heals. Attempts are bounded: once the deadline passes, the command fails with `command_dispatch_expired`. A `Failed` command whose reason says "delivery uncertain" → check the device queue in ChirpStack before re-issuing. |
| `command_dispatch_expired` | `warn` | Story J-1 iter-3 (D2): a `Pending` command exceeded the dispatch deadline (`global.command_dispatch_deadline_secs`, default **120 s** since [#182](https://github.com/guycorbaz/opcgw/issues/182) — its own knob, deliberately independent of the `command_delivery_timeout_secs` confirmation sweep — measured from `created_at`, **symmetric** since iter-4: a far-future `created_at` from a clock step-back also expires — `age_secs` is then negative) before it could be enqueued — marked `Failed`, **never delivered**. This is the safety gate that keeps a stale command (e.g. one carried across hours of downtime to a later boot's startup drain) from actuating hardware long after the operator wrote it. **Fields:** `command_id`, `device_id`, `age_secs`, `deadline_secs`. | Expected after any outage that outlived the deadline while commands were queued. The operator re-issues the command if still wanted. Frequent expiries *without* an outage → ChirpStack persistently unreachable (see `command_dispatch_retry`); a negative `age_secs` → host clock stepped back. |
| `command_dispatch_orphaned` | `warn` | Story J-1 iter-3 (D3): a `Pending` command's device/command is **no longer configured** (removed or re-ported via Apply after the row was queued) — marked `Failed`, never enqueued. Pre-iter-3 this fell back to a raw-byte unconfirmed downlink aimed at a device the operator had just removed. **Fields:** `command_id`, `device_id`, `f_port`. | Expected once per in-flight command when a device/command is deleted while commands are queued. Anything else → config drift; check the device's command list and `command_port`. |
| `command_dispatch_drain_cancelled` | `debug` | Story J-1 (iter-2 P6): a drain was interrupted by shutdown/Apply between commands; remaining `Pending` rows are deferred to the next generation's startup drain (delivered there if still within the delivery deadline). **Fields:** none. | None — normal teardown under a backlog. |
| `command_queue_row_invalid` | `warn` | Story J-1 iter-3 (D4, storage layer): `get_pending_commands` found a malformed `command_queue` row (unparseable `created_at`, out-of-range `f_port` — schema-version skew, manual edit, restored corrupt backup) and **quarantined** it: marked `Failed` with reason `malformed row quarantined: <detail>`. Valid rows still dispatch — pre-iter-3 one poison row failed the whole read and livelocked all command dispatch permanently. **Fields:** `command_id`, `reason`. Fires once per poison row (a `Failed` row is never re-read as `Pending`); only re-fires if the quarantine UPDATE itself failed. | Should be unreachable in normal operation. If seen: inspect the row by id via raw SQL; likely a restored backup or manual mutation. |
...
| `env_shadows_singleton_config` | `warn` (audit) | #169 (PR #170), narrowed by Story J-2 | Emitted once-per-boot by `AppConfig::maybe_warn_env_shadows_singleton` (from `main.rs` after a successful SQLite reload) for each **allowlisted** `OPCGW_<SECTION>__<KEY>` env var that coexists with a matching `singleton_config` row — for those keys env genuinely still outranks the Admin page. Fields: `env_var`, `section`, `key`, `env_value`, `db_value`. Since Story J-2, NON-allowlisted keys never reach this event (they no longer shadow anything — they are ignored and reported by `env_var_ignored` instead). Secret fields are never inspected or logged. |
| `env_var_ignored` | `warn` (audit) | Story J-2 (#169 enforcement) | Emitted once-per-boot (per key) by `AppConfig::maybe_warn_env_ignored` **immediately before** the bootstrap config load (deliberate: if the load then fails *because* a formerly env-supplied value is now ignored, the operator sees the cause next to the error): this `OPCGW_<SECTION>__<KEY>` env var addresses a **web/SQLite-managed field outside the env allowlist** and was EXCLUDED from the config — the web Admin page value is authoritative. Fields: `env_var`, `section`, `key`, `recommended_action` (the env VALUE is deliberately not logged). Short-form vars (`OPCGW_LOG_LEVEL`…), secrets, the bootstrap allowlist (`web.enabled/bind_address/port`, `opcua.host_port`) and sections outside `global/chirpstack/opcua/web` are never reported — they remain env-capable. Operator action: remove the override from `.env`/compose and manage the field on the Admin page. |
| `env_var_ignored_login_name` | `warn` (audit) | Story J-2 review iter-1 | Companion to `env_var_ignored` for the one ignored field that can lock an operator out: `[opcua].user_name` is also the **web-UI Basic-auth login name**, while `user_password` remains env-capable (it is a secret). When an ignored `OPCGW_OPCUA__USER_NAME` is present, this names the **effective** login user (`effective_user_name`) so the operator can sign in immediately. Fields: `env_var`, `effective_user_name`, `recommended_action`. Emitted once per boot, after the config load (it needs the effective value); the user name is explicitly not a secret (D-0 I1-F12). |
| `env_shadows_secrets_toml` | `warn` (audit) | Story J-2 review iter-1 | An allowlisted **secret** env var (`OPCGW_CHIRPSTACK__API_TOKEN` / `OPCGW_OPCUA__USER_PASSWORD`) and the wizard-written `config/secrets.toml` both supply the same field. The environment sits above `secrets.toml` in the figment stack, so credentials re-entered in the setup wizard have **no effect** until the env var is removed — previously silent (the shadow scan skips secrets, the ignored scan skips allowlisted keys). Fields: `section`, `key`, `recommended_action`. **Values are never logged.** Emitted from the config load, one line per affected field. |
...
- **Story J-1** (CR #136) — Decouple command dispatch from the metrics poll. A dedicated event-driven `CommandDispatcher` task (`src/chirpstack_dispatch.rs`), woken by the OPC UA write path via a `tokio::sync::Notify`, now owns the pending-command drain — so a command reaches ChirpStack within seconds of the write instead of on the next poll cycle. The former head-of-poll `process_command_queue` drain is removed. Adds the events catalogued above: `command_dispatch_drain` (debug, `count`), `command_dispatch_drain_error` (warn, `error`, self-driven escalating-backoff retry), `command_dispatch_retry` (warn — transient enqueue failure, row left `Pending` for a bounded retry), `command_dispatch_expired` (warn — delivery-deadline safety gate, `global.command_delivery_timeout_secs` from `created_at`), `command_dispatch_orphaned` (warn — device/command de-configured since queueing, never enqueued), `command_dispatch_drain_cancelled` (debug — drain interrupted by teardown), and the storage-layer `command_queue_row_invalid` (warn — malformed row quarantined `Failed` instead of failing the whole read). The task also logs a `debug` "Starting CommandDispatcher" line at start and an `info` "CommandDispatcher shutting down" line on graceful shutdown (SIGINT/SIGTERM or an Apply soft restart), mirroring the sibling `CommandStatusPoller` / `CommandTimeoutHandler` lifecycle lines.

...
| `uplink_metric_never_seen` | `warn` | Story E-1 (E-1b): after a few uplinks (`events_observed`), a configured `read_metric` (`metric=<chirpstack_metric_name>`) has **never** appeared in the device's decoded object, so it will not populate via the stream. Once per field per device. | The metric comes from DevStatus (e.g. battery) not the uplink object, OR `chirpstack_metric_name` doesn't match the codec's field name. Fix the config field name, get the codec to emit it, or accept it can't stream (keep that device on the poll). Key signal during the `stream_all_devices` fleet migration. |

...
| `metric_view_serialize` | `warn` | A-6 (FR51): defensive event from `metric_type_to_json_value` (the `/api/devices` dashboard wire path) when a `MetricType::Float` payload is non-finite (NaN / ±Inf) and therefore not representable as a bare JSON number. **Field schema (closed enum):** `event="metric_view_serialize"`, `reason="non_finite"`, `device_id=<str>`, `metric_name=<str>`, `f64_value=<f64>`. The dashboard receives `value: null` for the offending row (renders as "—" + "missing" badge). **Expected reachability:** unreachable in production because Story A-3's poller-side filter rejects non-finite Floats before they reach storage; the event is defensive against a future writer regression, a restored-backup bypass, or raw SQL injection. **Note:** post-A-5 `load_all_metrics` silently skips `value_type='legacy'` rows BEFORE they reach the dashboard handler, so there is NO `legacy_row` reason here — legacy rows surface as the same "configured but never polled" outcome at the wire (`value: null, timestamp: null`). | If this event ever fires in production, the A-3 poller filter has regressed OR the database was mutated outside the gateway. Inspect `metric_history` for the same `device_id` + `metric_name` to see if the non-finite write came through `metric_values` UPSERT or via direct raw SQL. |

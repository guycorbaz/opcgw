...
6. **Docker bind mounts preserve `opcgw.db` across container replacement.** Path A works transparently with the standard `docker compose pull && docker compose up -d` workflow because the database file lives on the host filesystem (per the `docker-compose.yml` volume mount). Path B requires removing the file from the host side (`rm ./opcgw.db*` in the directory that's bind-mounted).

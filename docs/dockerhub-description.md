...
- **Atomic schema migrations** — versioned (`v001`–`v013`), per-startup forward-only
...
|--------------------|-----------|--------------------------------------------------------------------|
| `<major>.<minor>.<patch>` | `2.5.0`  | The exact release. Pin to this for fully reproducible deployments. |
| `<major>.<minor>`         | `2.5`    | Latest patch within a minor line. Auto-updates on patch releases.  |
...
| `manifest unknown` on `gcorbaz/opcgw:2.0.0` | No image was ever published for v2.0.0 (workflow bug; see the "Supported tags" note above) | Pull `:2.5` (recommended) or `:2.5.0` (exact) instead |

...
|--------------------|-----------|--------------------------------------------------------------------|
| `<major>.<minor>.<patch>` | `2.1.0`  | The exact release. Pin to this for fully reproducible deployments. |
| `<major>.<minor>`         | `2.1`    | Latest patch within a minor line. Auto-updates on patch releases.  |
...
| `manifest unknown` on `gcorbaz/opcgw:2.0.0` | No image was ever published for v2.0.0 (workflow bug; see the "Supported tags" note above) | Pull `:2.1` (recommended) or `:2.1.0` (exact) instead |

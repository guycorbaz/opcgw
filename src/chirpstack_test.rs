...
async fn test_list_devices(chirpstack_client: &mut ChirpstackClient) {
    ...
    match chirpstack_client
        .list_devices("194f12ab-d0ab-4389-a446-f1b3e7152b07".to_string())
        .await
    {
        Ok(devices) => {
            ...
            if let Some(first_device) = devices.first() {
                match chirpstack_client
                    .get_device_details(first_device.dev_eui.clone())
                    .await
                {
                    Ok(metrics) => {
...
async fn test_get_device_metrics(chirpstack_client: &mut ChirpstackClient) {
    match chirpstack_client
        .get_device_metrics("a840414bf185f365".to_string(), 1, 100)
        .await
    {
        Ok(device) => {
